...
import { Smartphone, CheckCircle2, ArrowRight, MapPin, Phone } from 'lucide-react';
import LetsTalkButton from './LetsTalkButton';

export default function MobileAppMalegaon({ onOpenContact, onNavigate }) {
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Mobile App Development Company in Malegaon | Sparrow IT & Digital Solutions";
  ...
  const features = [
    { title: 'Android App Development', desc: 'Custom Android applications published directly to Google Play Store with optimal device compatibility.' },
    { title: 'iOS App Development', desc: 'Native and cross-platform iOS mobile apps designed for Apple iPhone and iPad users.' },
    { title: 'Push Notifications & Auth', desc: 'Integrated real-time user authentication, OTP verification, and automated push notifications.' },
    { title: 'Payment Gateway Integration', desc: 'Secure Indian payment gateway integration (UPI, Razorpay, PhonePe, Cards) for seamless transactions.' },
  ];
    ...
    <div className="pt-28 pb-20 bg-[#0A2540] text-slate-100 min-h-screen font-sans">
      <section className="px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto text-center space-y-6">
        ...
        <p className="text-base sm:text-xl text-blue-100 max-w-3xl mx-auto leading-relaxed">
          Sparrow IT &amp; Digital Solutions designs and develops Android &amp; iOS mobile applications tailored for businesses and enterprise workflows in Malegaon.
        </p>

        <div className="flex flex-wrap items-center justify-center gap-4 pt-2">
          <LetsTalkButton onClick={() => onOpenContact('Mobile App Development in Malegaon')} label="Start Your App Project" size="md" />
          <button
      ...
      <section className="mt-16 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto text-center space-y-6">
        <div className="p-8 rounded-2xl bg-gradient-to-r from-[#092B80] to-[#1866EC] text-white space-y-4">
          <h2 className="text-2xl font-extrabold">Build Your Mobile App with Sparrow</h2>
          <p className="text-xs text-blue-100 max-w-md mx-auto">Turn your app idea into a high-performance Android or iOS mobile application.</p>
          <div className="pt-2 flex justify-center">
            <LetsTalkButton onClick={() => onOpenContact('Mobile App Development in Malegaon')} label="Discuss Mobile App" size="md" />
          </div>
        </div>
      </section>
    </div>
